...
SetStep.prototype.init=function(arg){
    ...
    var stepP=$("<div class='ystep-progress'>"+
            ...
            "</div>");
    var stepButtonHtml =$( "<div class='step-button'>"+
                        "<button type='button' class='btn btn-green nextBtn pull-right' id='nextBtn' >下一步</button></div>");
    ...
    if(this.opt.showBtn){
        ...
        this.content.find(".ystep-container").after(stepButtonHtml)
    ...
    if(this.opt.clickAble){
        stepsHtml.find('li').on('click',function(){
            if($(this).hasClass("ystep-step-done") || $(this).hasClass("ystep-step-active")){
                var i = $(this).index()+1;
               _that.pageCont.find('#page'+i).css("display","block");
               _that.pageCont.find('#page'+i).siblings().css("display","none");
            }else{
                return false;
            }
            console.log($(this))
